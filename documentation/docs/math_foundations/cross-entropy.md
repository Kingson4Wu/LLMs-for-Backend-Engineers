...
- [Softmax](./softmax.md) — Softmax 将 logits 转换成概率分布，交叉熵对这个分布进行评估，两者组合是分类任务的标配
- [反向传播](./backpropagation.md) — 交叉熵 + Softmax 的组合有一个极为优雅的特性：梯度恰好是 p_i - y_i（预测概率减去真实概率），是反向传播中梯度计算简洁的根本原因
