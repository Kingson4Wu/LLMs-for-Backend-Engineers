...
点积的**原始定义**（代数形式）其实非常简单：

**a · b = a₁b₁ + a₂b₂ + ... + aₙbₙ**

就是**对应坐标相乘再求和**。这个定义里，每一项都包含了 b 的分量信息。
...
- [从 One-hot 到 Embedding：词的分布式表示](../../embeddings/from-onehot-to-embedding) — Embedding 如何用向量表示词的语义
- [Attention 机制](../../advanced_transformer/attention_mechanism) — 点积在 Attention 中用于计算 Query 与 Key 的相似度