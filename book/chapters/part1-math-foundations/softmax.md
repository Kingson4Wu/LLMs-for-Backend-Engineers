...
Softmax 出现在分类输出和注意力权重中。它接收一组未归一化分数（logits），输出总和为 1 的相对权重；某个分数上升会同时改变其他候选项的权重。这一章解释这个映射为何满足概率约束、为什么使用指数，以及它如何和交叉熵连接。
...
第 4 点把可相加的分数转换为可相乘的相对权重，也是 Softmax 能与概率建模相连的关键。
...
若用其他正函数 $g(x)$ 替代指数，梯度一般会出现 $g^{\prime}(x)/g(x)$ 项，不再得到交叉熵下简洁的 $p_i-y_i$ 形式；这不等于其他归一化一定无法优化。

---

## 从加法分数到概率比值

这是解释 Softmax 的一条重要结构线索，不是说它在所有任务中都是唯一的选择。
...
在要求 $g$ 为正、连续并满足这种乘法一致性的条件下，解为指数形式：
...
**结论**：在上述一致性条件下，指数把 logit 的加法差值转成概率的乘法比值；这说明它与对数几率模型十分契合，并不排除其他任务使用其他归一化方式。
...
在“概率和为 1、期望分数固定”的约束下，最大熵分布是指数形式。它提供了 Softmax 的一条信息论推导；约束来自建模假设，并不证明 Softmax 是所有概率建模任务中唯一正确的归一化。

---

## 小结：Softmax 的作用与边界

Softmax 是分类和自回归生成中很常用的选择，因为它同时具备以下性质：
...
**一句话**：Softmax 用指数把可比较的分数转换为归一化概率，并与交叉熵形成简洁的训练接口；它很常用，但不是唯一可用的归一化设计。
