# 从 One-hot 到 Embedding：离散 Token 怎样成为向量

在自然语言处理任务中，模型面对的基本对象是离散的符号（词或 token），而大多数机器学习方法只能处理连续数值。因此，一个核心问题是：**如何将离散的词表示为数值向量，并使这些向量能够反映词的语义与用法差异**。

词向量（word embedding）是对这一问题的系统性回答。它先把离散 ID 映射为可学习的连续表示；在早期模型中，这种表示可由共现预测学习，而在 Transformer 中它是后续上下文计算的起点。这里以 Skip-gram 和 Negative Sampling 为例，说明向量为何会从语料中形成。

---

## One-hot 表示：离散符号的直接编码
...
## Skip-gram 与 Negative Sampling：从语料到训练目标
