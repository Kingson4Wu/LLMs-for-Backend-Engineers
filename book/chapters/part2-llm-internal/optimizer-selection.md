...
这是一种基于梯度一阶矩与平方梯度二阶矩的自适应一阶方法；这里的“二阶矩”不是 Hessian，不能把 Adam 等同于二阶优化。
...
| 参数类型 | 梯度特征 |
|---------|---------|
| Embedding | 输入查表时常呈稀疏更新；若与输出层权重共享或计算完整 Softmax，梯度也可能变得稠密 |
...
**效果**：
- 同一 weight-decay 参数组中的参数受到一致的衰减比例；实践中常将偏置和归一化参数排除在该组之外
- 泛化性能更稳定
...
>
> **这三者是常见的 Transformer 训练配方；具体是否采用、怎样设置，仍取决于模型、数据、规模与训练目标。**
